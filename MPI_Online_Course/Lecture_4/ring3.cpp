...
Slightly faster than Send_recv. Close to latency.
*/
...
int main(){
	...
	void *bsend_buf = NULL;
	int bufsize = 1024*64; //just because
	...
	for(unsigned k=0; k<repeat; ++k){
		...
		for(int i=1; i<size; ++i){
			int recv;
			// MPI_Bsend(&send1, 1, MPI_INT, next, rank, MPI_COMM_WORLD);
			MPI_Recv (&recv , 1, MPI_INT, prev, prev, MPI_COMM_WORLD, &status);

			sum  += recv;
			send1 = recv;

			// MPI_Bsend(&send2, 1, MPI_INT, next, rank, MPI_COMM_WORLD);
			MPI_Send(&send2, 1, MPI_INT, next, rank, MPI_COMM_WORLD);
			MPI_Recv (&recv , 1, MPI_INT, prev, prev, MPI_COMM_WORLD, &status);
